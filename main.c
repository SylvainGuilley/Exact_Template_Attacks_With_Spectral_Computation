...
float xmoyentran[NBMESURES][256]; //Xcumul
float segmainverse_fois_ymoyen[NBMESURES][256];
float ymoyentrans_fois_segmainverse_fois_ymoyen[256]; //ycal
...
//calcul de ymoyentrans_fois_segmainverse_fois_ymoyen
for (i=0;i<256;i++){ ymoyentrans_fois_segmainverse_fois_ymoyen[i]=0.0;}

for (i=0;i<256;i++){
for (j=0;j<NBMESURES;j++)
						{
 ymoyentrans_fois_segmainverse_fois_ymoyen[i]+=ymoyen[j][i]*segmainverse_fois_ymoyen[j][i]; //Ycal
                        }
                    }
//fin de calcul de ymoyentrans_fois_segmainverse_fois_ymoyen

for(j=0;j<NBMESURES;j++){
				//Fourier_float(xmoyentran[j],8);
				Fourier_float(segmainverse_fois_ymoyen[j],8);
				}
Fourier_float(ymoyentrans_fois_segmainverse_fois_ymoyen,8);
...
for(i=0;i<256;i++){
	for(j=0;j<NBMESURES;j++)
	{
	  xmoyentran_fourier[j][i]=xmoyentran[j][i]; //Xcumul
	}
			}

for(i=0;i<256;i++){n_fourier[i]=(float)n[i];
}
...
for(i=0;i<256;i++){
	for(j=0;j<NBMESURES;j++)
	{
	  somme_des_produits[i]+=xmoyentran_fourier[j][i]*segmainverse_fois_ymoyen[j][i]; //\sum wht (Xcumul). wht(Y)
	}
			}

Fourier_float(n_fourier,8);

for(i=0;i<256;i++)n_fourier[i]*=ymoyentrans_fois_segmainverse_fois_ymoyen[i]; //wht(n) .wht(Ycal)
for(i=0;i<256;i++){somme_des_produits[i]=n_fourier[i]-2*somme_des_produits[i];} //wht(n) .wht(Ycal) -2 \sum wht (Xcumul). wht(Y)
Fourier_float(somme_des_produits,8); //wht-1 global
//t2 = clock();
...
for(i=1;i<256;i++)
		...
		if (somme_des_produits[i]<m) //Minimiser au lieu de maximiser
			{
